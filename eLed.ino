// =========================================================================================
// 
// eLed v1.00  Software for displaying temperature received via API
//
...
// Current version of the eLed software
float g_current_version = 1.00;
...
// ESP8266 settings
const char*       g_AP_ssid              = "eLed_Device";   // Default Access Point ssid
const char*       g_AP_password          = "eLed_Device";   // Default Password for AP ssid

const char*       g_default_host_name    = "eLed_Device";   // Default Hostname
...
// Globals containing supplied (API) temperature, humidity and pressure (if available)
...
void setup() 
{
  ...
  if ( (int)g_data.version != (int)g_current_version )
  {
    ...
    g_data.display_type              = -1;      // 0 = MAX7219, 1 = Oled 128x64 (default)
  ...
  // If connected in WiFi mode, display the eLed Web-Interface
  else
  {
    Serial.println("");
    Serial.println("Connected in WiFi mode thus display eLed Web-Interface");
    server.on ( "/", handle_root_ajax ); 
  }

  // Setup supported URL's for this ESP8266
  server.on("/erase", handle_erase );                       // Erase EEPROM
  server.on("/api", handle_api);                            // API of eLed
...
void handle_api()
{
  ...
  else
  {
    ...
    else if (action == "set_language")
    {
      ...
      // Make sure that value is supplied for language
      if (value == "")
      {
    ...
    {
      Serial.println("Handle 'set_temperature' action");

      // Make sure that value is supplied for temperature
      if (value == "")
      {
        Serial.println("No value supplied for 'set_temperature' action, ignoring API");
     
        server.send ( 501, "text/html", "NOK (no temperature supplied for 'set_temperature' action, ignoring API");
        delay(200);
      }
      else
      {
        ...
        g_temperature  = value.toFloat();

        char buffer [256];
        sprintf( buffer, "OK (temperature set to value '%1.1f')", g_temperature );
    ...
    {
      Serial.println("Handle 'set_humidity' action");

      // Make sure that value is supplied for humidity
      if (value == "")
      {
        Serial.println("No value supplied for 'set_humidity' action, ignoring API");
     
        server.send ( 501, "text/html", "NOK (no humidity supplied for 'set_humidity' action, ignoring API");
        delay(200);
      }
      else
      {
        ...
        g_humidity  = value.toFloat();

        char buffer [256];
        sprintf( buffer, "OK (humidity set to value '%1.1f')", g_humidity );
    ...
    {
      Serial.println("Handle 'set_pressure' action");

      // Make sure that value is supplied for pressure
      if (value == "")
      {
        Serial.println("No value supplied for 'set_pressure' action, ignoring API");
     
        server.send ( 501, "text/html", "NOK (no pressure supplied for 'set_pressure' action, ignoring API");
        delay(200);
      }
      else
      {
        ...
        g_pressure  = value.toFloat();

        char buffer [256];
        sprintf( buffer, "OK (pressure set to value '%1.1f')", g_pressure );
...
bool is_authenticated(int set_cookie)
{
  ...
  String valid_cookie = "eLedDevice=" + web_pass_md5;

  // Check if web-page has Cookie data
  if (server.hasHeader("Cookie"))
  {
    // Check if Cookie of web-session corresponds with our Cookie of eLed
    ...
    if (cookie.indexOf(valid_cookie) != -1)
    {
      ...
      if (set_cookie == 1)
      {
        server.sendHeader("Set-Cookie","eLed_Device=" + web_pass_md5 + "; max-age=86400");
      }
      return true;
    }
  }
...
void handle_login_ajax()
{
  ...
  if (form == "login")
  {
    ...
    {
      String header = "HTTP/1.1 301 OK\r\nSet-Cookie: eLedDevice=" + web_pass_md5 + "; max-age=86400\r\nLocation: /\r\nCache-Control: no-cache\r\n\r\n";
...
{ 
  ...
  page += "<title>eLed</title>";
  page += "<style> body { background-color: #fffff; font-family: Arial, Helvetica, Sans-Serif; Color: #000088; }</style>";
  page += "</head>";
  
  page += "<body><h1>eLed Web-Interface</h1>";
...
void handle_root_ajax()
{
  ...
  if ( !is_authenticated(0) )
  {
    Serial.println("Unauthorized to access Web-Interface. Reset Cookie and present login screen.");
...
// -----------------------------------------------------------------------------------------
// History of 'eLed' software
//
// 09-Jan-2022  V1.00   Initial version
// -----------------------------------------------------------------------------------------
